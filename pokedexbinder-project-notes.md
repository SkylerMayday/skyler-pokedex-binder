...
- [x] Spec for binder page-flip viewer (web) — **SUPERSEDED 2026-07-09**, not being built here. Equivalent feature now owned by the website project (skylermayday-site) natively — see Key Decisions.
...
## Key Decisions
- **Display/design ownership for the public binder collection is now the website project's, not this project's** (Skyler, 2026-07-09): the approved `2026-07-08-binder-page-flip-viewer.md` spec (bookshelf + 3×3 page-flip viewer) is superseded — the same interaction is being built natively on the new SkylerMayday.com site (`D:\Claude Projects\skylermayday-site`) instead of on this project's GitHub Pages viewer. This project (PokedexBinderV2) retains ownership of the Android app and the publish pipeline (binder.json/changelog.json schema) only — not how that data is visually presented to the public. The GitHub Pages viewer (`web/pokedex-binder-site/`) stays live in maintenance-only mode (no new design/feature work) until the new site's redesign ships and is verified, then retires per the original site spec's phasing.
